...
thisFilename = 'op_dyn_sbm' # This is the general name of all related files
...
transfData = op_dyn.OpinionDynamics(S_large,mu,sigma)
xTest2, yTest2 = transfData.getData(nTest)
xTest2 = np.expand_dims(xTest2,axis=2)
yTest2 = np.expand_dims(yTest2,axis=2)
...
for rlz in range(n_realizations):
    ...
    for idx,n in enumerate(n_vector):
        ...
        originalData = op_dyn.OpinionDynamics(S,mu,sigma)
        xTrain, yTrain = originalData.getData(nTrain)
        xTrain = np.expand_dims(xTrain,axis=2)
        yTrain = np.expand_dims(yTrain,axis=2)
        xValid, yValid = originalData.getData(nValid)
        xValid = np.expand_dims(xValid,axis=2)
        yValid = np.expand_dims(yValid,axis=2)
        xTest, yTest = originalData.getData(nTest)
        ...
        for m_idx, model in enumerate(modelList):
            ...
            xPlot = torch.tensordot(featsTest2[0].squeeze().cpu(),V[:,1],dims=([1],[0]))
            ind = np.argsort(xPlot)
            x_label[rlz,:] = xPlot[ind]
            yPlot = torch.tensordot(kernel_preds2.squeeze().cpu(),V[:,1],dims=([1],[0]))
            yPlot2 = torch.tensordot(gnn_preds2.squeeze().cpu(),V[:,1],dims=([1],[0]))
            yPlot3 = torch.tensordot(yTest2.squeeze().cpu(),V[:,1],dims=([1],[0]))
            ...
            plt.xlabel('$[\hat{\mathbf{x}}]_2$, sorted')
            plt.ylabel('$[\hat{\mathbf{y}}]_2$')
